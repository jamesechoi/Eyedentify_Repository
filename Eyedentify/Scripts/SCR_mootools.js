...
var Native = function (K) {
    ...
    if (I && F) { D.prototype = I.prototype } D.prototype.constructor = D;
    if (A) {
        var E = A.toLowerCase();
        ...
        if (H) { Native.genericize(N, L, B) } G.call(N, L, O);
        return N
    };
...
Native.typize = function (A, B) {
    if (!A.type) { A.type = function (C) { return ($type(C) === B) } }
};
(function () {
...
Hash.implement({ forEach: function (B, C) {
    ...
    var B = {};
    for (var A in this) {
        if (this.hasOwnProperty(A)) { B[A] = this[A] }
    } return B
}, getLength: function () {
    var B = 0; for (var A in this) {
        if (this.hasOwnProperty(A)) { B++ }
    } return B
}
});
...
function $clear(A) {
    clearTimeout(A); clearInterval(A);
    return null
}
function $defined(A) {
    return (A != undefined)
...
Browser.detect = function () {
    for (var B in this.Engines) {
        ...
        if (A) {
            ...
            this.Engine[B] = this.Engine[B + A] = true; break
        }
    }
...
var Window = new Native({ name: "Window", legacy: (Browser.Engine.trident) ? null : window.Window, initialize: function (A) {
    ...
    if (!A.Element) {
        A.Element = $empty;
        if (Browser.Engine.webkit) { A.document.createElement("iframe") } A.Element.prototype = (Browser.Engine.webkit) ? window["[[DOMElement.prototype]]"] : {}
    } A.document.window = A;
...
new Document(document); Array.implement({ every: function (C, D) {
    ...
    for (var B = 0, A = this.length;
      B < A; B++) { C[B] = D.call(E, this[B], B, this) }
    return C
}, some: function (C, D) {
    for (var B = 0, A = this.length; B < A; B++) {
        if (C.call(D, this[B], B, this)) {
    ...
    for (var B = 0, A = this.length; B < A; B++) {
        var C = $type(this[B]);
        if (!C) { continue } D = D.concat((C == "array" || C == "collection" || C == "arguments") ? Array.flatten(this[B]) : this[B])
    } return D
}, hexToRgb: function (B) {
    if (this.length != 3) {
        return null
    ...
    var A = this.map(function (C) {
        if (C.length == 1) { C += C } return C.toInt(16)
    });
    return (B) ? A : "rgb(" + A + ")"
}, rgbToHex: function (D) {
    if (this.length < 3) {
...
Number.implement({ limit: function (B, A) {
    ...
    for (var A = 0; A < this; A++) { B.call(C, A, this) }
}, toFloat: function () {
...
String.implement({ test: function (A, B) {
    ...
    var A = ""; var C = this.replace(/<script[^>]*>([\s\S]*?)<\/script>/gi, function () {
        A += arguments[1] + "\n";
        return ""
    });
    if (B === true) { $exec(A) }
...
Hash.implement({ has: Object.prototype.hasOwnProperty, keyOf: function (B) {
    ...
    if (this.hasOwnProperty(A)) { delete this[A] }
    return this
}, get: function (A) {
    ...
    var A = this[B]; if (A == undefined) { this[B] = C }
    return this
}, map: function (B, C) {
    var A = new Hash;
    Hash.each(this, function (E, D) { A.set(D, B.call(C, E, D, this)) }, this);
    return A
}, filter: function (B, C) {
    var A = new Hash; Hash.each(this, function (E, D) {
        if (B.call(C, E, D, this)) { A.set(D, E) }
    }, this);
    return A
}, every: function (B, C) {
    for (var A in this) {
    ...
    var A = []; Hash.each(this, function (C, B) { A.push(B) });
    return A
}, getValues: function () {
    var A = [];
    ...
    var B = []; Hash.each(this, function (F, E) {
        ...
        switch ($type(F)) {
            case "object": D = Hash.toQueryString(F, E);
                break;
            case "array": var C = {}; F.each(function (H, G) { C[G] = H });
...
var Event = new Native({ name: "Event", initialize: function (A, F) {
    ...
    if (A.$extended) { return A } this.$extended = true;
    var J = A.type; var G = A.target || A.srcElement;
...
Event.implement({ stop: function () {
    ...
    A.constructor = Class; A.prototype = B;
    return A
}
});
Class.Mutators = { Extends: function (C, A) {
    ...
    var B = new A;
    delete B.parent; B = Class.inherit(B, C);
    delete Class.prototyping; return B
...
Class.extend({ inherit: function (B, E) {
    ...
    for (var D in E) {
        ...
        if (G && F == "function") {
            if (C != G) {
                if (A) { C.__parent = G; B[D] = C }
                else { Class.override(B, D, C) }
            }
        }
        else {
            if (F == "object") { B[D] = $merge(G, C) }
            else { B[D] = C }
        }
    } if (A) {
        B.parent = function () {
    ...
    if (D && B[A] != D[A]) { D = null }
    var C = function () {
        var F = this.parent; this.parent = D ? D[A] : B[A];
...
Class.implement({ implement: function () {
    var A = this.prototype; $each(arguments, function (B) { Class.inherit(A, B) });
    return this
}
});
...
var Events = new Class({ $events: {}, addEvent: function (C, B, A) {
    ...
    for (var D in this.$events) {
        if (C && C != D) { continue } var B = this.$events[D];
...
var IFrame = new Native({ name: "IFrame", generics: false, initialize: function () {
    ...
    var A = function () {
        var F = $try(function () { return B.contentWindow.location.host });
...
var Elements = new Native({
    initialize: function (F, B) {
        B = $extend({ ddup: true, cash: true }, B); F = F || [];
        if (B.ddup || B.cash) {
            var G = {}, E = [];
...
Elements.implement({ filter: function (A, B) {
    ...
    if (Browser.Engine.trident && B) {
        ["name", "type", "checked"].each(function (C) {
            if (!B[C]) { return } A += " " + C + '="' + B[C] + '"'; if (C != "checked") {
                delete B[C]
            }
        }); A = "<" + A + ">"
    }
...
}, getDocument: function () { return this.document }, getWindow: function () {
    return this
}
...
$.element = function (A, D) {
    ...
    if (!D && !A.$family && !(/^object|embed$/i).test(A.tagName)) {
        var B = Element.Prototype; for (var C in B) { A[C] = B[C] }
    }
    return A
...
(function () {
    ...
    var G = function (N, L) {
        if (!N) { return } var M = N.uid;
        if (Browser.Engine.trident) {
            if (N.clearAttributes) {
                var P = L && N.cloneNode(false); N.clearAttributes(); if (P) { N.mergeAttributes(P) }
            }
            else {
    ...
    Hash.each(A, function (L, M) {
        ...
        set: function (O, M) {
            switch ($type(O)) {
                case "object": for (var N in O) { this.set(N, O[N]) }
            ...
            if (L && B[M]) { N = !!N } (L) ? this[L] = N : this.setAttribute(M, "" + N); return this
            ...
            L = $(L, true); L.parentNode.replaceChild(this, L);
            return this
        }, wraps: function (M, L) {
            M = $(M, true);
            ...
            if (!M) { return null } for (var L = M.parentNode; L != this;
            L = L.parentNode) {
                if (!L) { return null }
            }
            return $.element(M, N)
            ...
            if (this.currentStyle) { return this.currentStyle[M.camelCase()] }
            ...
            Element.empty(this);
            Element.dispose(this); G(this, true);
            return null
        }, empty: function () {
            $A(this.childNodes).each(function (L) { Element.destroy(L) });
            return this
...
Element.Properties.html = (function () {
    ...
    A.thead = A.tfoot = A.tbody; var B = { set: function () {
        ...
        if (F) {
            var G = C; G.innerHTML = F[1] + E + F[2]; for (var D = F[0]; D--; ) { G = G.firstChild } this.empty().adopt(G.childNodes)
        }
        else {
            this.innerHTML = E
        }
...
Native.implement([Element, Window, Document], { addEvent: function (E, G) {
    ...
    removeEvent: function (C, B) {
        ...
        if (D) {
            if (D.onRemove) { D.onRemove.call(this, B) } C = D.base || C
        }
